...
###調整客群參照讀取時，每次連線postgresql，更改為服務啟動後，就讀一次整張客群參照表
SQL = 'Select tag_info from second_edition '
cur.execute(SQL)
seg_records = cur.fetchall()
def getSeg_ref_v1(tag_value):
    seg_qresult = [t[0] for t in seg_records if t[0]['Tag_Value']==tag_value ][0]
    return seg_qresult
###
...
def get_fx_rec(vid):
    ...
    #business rule，依照客群標籤1,2,其他三類區分顧客服務階級
    if 'SEG001' in tag_list : 
        ...
        offer1 = [u'OFF0004']
        #model rule
    #fx_pag_tag：顧客瀏覽的頁面所對應的標籤清單，所以是外匯走勢頁的offer tag
    #fx_order：所有可能推薦的外匯offer清單
...
#連Tag_server 取得user tag
def get_user_tag(user_id):
    TAG_SERVER = 'tagserver'
    url = "http://%s/GetAllTag"%(TAG_SERVER,)
    payload = {'vid':user_id}
    response = requests.get(url, params=payload)
    user_tags_res = json.loads(response.content)
    return user_tags_res
...
def getSeg_ref(table,tag_value,sql_db):
    ...
    return records
#取得postgresql中各個offer 的 offer tag
def getoff_ref(table,tag_value,sql_db):
    SQL = "Select off_info from " + table + " where off_info ->> 'OFFER_ID' = '{}'".format(tag_value)
    cur.execute(SQL)
    records = cur.fetchall()[0][0]
    return records
#consine simularity，因為在這次標籤的世界中，沒有使用到tagvalue，i.e. 全部都是0,1的世界，所以兩個向量的長度，可簡化為兩個向量的元素個數
#而內機則可簡化為交集的長度
